...
export const addUpdatePermissions = createAsyncThunk(
    ...
    async ({role_id,user_id, permissions}, {rejectWithValue}) => {
        try {
            const response = await api.post(`/api/v1/users/add_or_update/permissions?role_id=${role_id || ''}&user_id=${user_id || ''}`, {
                permissions,
            });
...
export const getRolePermissions = createAsyncThunk(
    ...
    async (data, {rejectWithValue}) => {
        try {
            const response = await api.get(`/api/v1/users/get/permissions?role_id=${data?.role_id || ''}&user_id=${data?.user_id || ''}`);
            return response?.data;
        } catch (error) {
